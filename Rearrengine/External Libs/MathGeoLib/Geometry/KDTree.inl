	...
	while (currentNode) {
		while (!currentNode->IsLeaf()) // while currentNode is an internal node (not a leaf)
		{
			...
			if (stack[entryPoint].pos[axis] <= splitPos) {
				if (stack[exitPoint].pos[axis] <= splitPos) { // Cases N1,N2,N3,P5,Z2 and Z3.
					currentNode = &nodes[currentNode->LeftChildIndex()];
					continue;
				}
				if (EqualAbs(stack[exitPoint].pos[axis], splitPos)) { // Case Z1
				...
				if (splitPos < stack[exitPoint].pos[axis]) { // Cases P1,P2,P3 and N5
					currentNode = &nodes[currentNode->RightChildIndex()];
					continue;
				}
				// Case P4:
